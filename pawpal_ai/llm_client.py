...
`LLMClient` protocol below. Three implementations exist:

* `AnthropicLLMClient` - the real provider adapter (lazy-imports `anthropic`,
  so the package is only required in live mode).
...
def create_client_from_env():
    """Build the configured client: live Anthropic adapter or offline demo.
    ...
    if live_mode_enabled():
        return AnthropicLLMClient(), "live"
